...
- Studied two existing projects as patterns to follow:
  ...
  - **dynalist_export**: FTS5 with triggers for auto-sync, hierarchical data with `parent_id`/`depth`/`path` columns, `sync_state` table for incremental imports
...
- Implement the JSON-to-SQLite import code that walks the export directory tree and populates the database
- Add a CLI flag (e.g. `--to-sqlite`) to trigger the import
- Eventually: add a search CLI or MCP server for querying the database
